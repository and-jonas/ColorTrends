...
selected = pd.read_csv(f'{out_dir}/Meta/selected_for_vegcol_annotations.csv')
...
for image in full_names_sel:
    ...
    # save patch
    imageio.imwrite(f'{out_dir}/patches_annotation_vegcol/{stem_name}.png', patch)
    # save coordinates
    df = pd.DataFrame([c])
    df2 = df.set_axis(['x1', 'x2', 'y1', 'y2'], axis=1, inplace=False)
    df2.to_csv(f'{out_dir}/patches_annotation_vegcol/coordinates/{stem_name}.txt', index=False)
